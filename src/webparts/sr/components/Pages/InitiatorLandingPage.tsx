...
export const InitiatorLanding: React.FunctionComponent<ISrProps> = (props: ISrProps) => {
  ...
  const onBlockRequestSearch = async (filterValue: string) => {
    ...
    setUtility(util);
    const filtered = await util.filterData(allData, filterValue, ["ProjectCode","PlantCode","TypeofItem","Location","ItemDescription","UOM","NewConsumedQtyuptopreviousMonths","RecdQtyUpTopreviousMonthsinceinc","ScrapSoldOutQtyuptopreviousMonth","ScrapSoldOutValue","CurrentMonthOpeningQty","CurrentMonthRecdQty","CurrentMonthSoldQty","CurrentMonthScrapSoldValueINRexc","TotalRecdQty","TotalSold","ScrapAvailable","TotalSoldOutValueINRexclTax"]);    
  ...
  const handleDelete = async (itemId) => {
    ...
    if (confirmed) {
      ...
      await sp.deleteData("ScrapReport", itemId, props);
      alert("Deleted");
              ...
              <table className={`${styles.tblrequest} ${styles.tablebordered}`}>
                <thead>
                  <tr>
                    ...
                    <th>UOM</th>
                    <th>Consumed Qty Prev Month</th>
                    <th>Recd Qty Prev Month</th>
                    <th>Scrap Sold Qty Prev Month</th>
                    <th>Scrap Sold Value Prev Month</th>
                    <th>Current Month Opening Qty</th>
                    <th>Current Month Recd Qty</th>
                    <th>Current Month Sold Qty</th>
                    <th>Current Month Scrap Sold Value</th>
                    <th>Total Recd Qty</th>
                    <th>Total Sold</th>
                    <th>Scrap Available</th>
                    <th>Total Sold Out Value</th>
                ...
                {currentPageData && currentPageData.length > 0 ? (
                  currentPageData.map(row => (
                    <tr key={row.Id}>
                      ...
                      <td>{row.Location}</td>
                      <td>{row.TypeofItem}</td>
                      <td>{row.ItemDescription}</td>
                      <td>{row.UOM}</td>
                      <td>{row.NewConsumedQtyuptopreviousMonthsinceinception}</td>
                      <td>{row.RecdQtyUpTopreviousMonthsinceinception}</td>
                      <td>{row.ScrapSoldOutQtyuptopreviousMonthsinceinception}</td>
                      <td>{row.ScrapSoldOutValue}</td>
                      <td>{row.CurrentMonthOpeningQty}</td>
                      <td>{row.CurrentMonthRecdQty}</td>
                      <td>{row.CurrentMonthSoldQty}</td>
                      <td>{row.CurrentMonthScrapSoldValueINRexclTax}</td>
                      <td>{row.TotalRecdQty}</td>
                      <td>{row.TotalSold}</td>
                      <td>{row.ScrapAvailable}</td>
                      <td>{row.TotalSoldOutValueINRexclTax}</td>
                      <td>{row.Editor}</td>
