...
export const EditRequest: React.FunctionComponent<IWslProps> = (props: IWslProps) => {
  ...
  const [workrequestColl, setPedningdata] = useState<IWSLMaster[]>();
  ...
  let BrandText: any;
    ...
    async function onRequestInitiate(formValues: any) {
        ...
        let PRRequest: any = {                   
          ...
          'ItemCodeId':formValues.ItemCodeId,
          ...
          'Lifeleftindays':formValues.Lifeleft,
        ...
        await spCrudObj.updateData("ShelfLife", workrequestColl[0].Id, PRRequest, props).then(async (brrAppUpdateResult1) => {

        alert ("WSL data updated successfully")
    ...
    React.useEffect(() => {
        ...
        IWSLRequestsOps().getWSLDatafilter(ArtIntId,props).then((WSLData) => {
          setPedningdata(WSLData);
    ...
    return (
      <Formik initialValues={initialvalues}
        ...
        onSubmit={(values, helpers) => { }}>{
            ...
            <div>
              {
                React.useEffect(() => {
                  if (workrequestColl && workrequestColl.length > 0) {
                    const data = workrequestColl[0];
                    formik.setFieldValue('ExpiryDate', data.ExpiryDate?.split('T')[0]);
                    formik.setFieldValue('PlantCodeId', data.PlantCodeId);
                    formik.setFieldValue('ProjectCodeId', data.ProjectCodeId);
                    formik.setFieldValue('ItemCodeId', data.ItemCodeId);
                    formik.setFieldValue('ProjectName', data.ProjectName);
                    formik.setFieldValue('Location', data.Location);
                    formik.setFieldValue('ItemDescription', data.ItemDescription);
                    formik.setFieldValue('UOM', data.UOM);
                    formik.setFieldValue('HandQty', data.OnHandQty);
                    formik.setFieldValue('Value', data.Value);
                    formik.setFieldValue('Utilization', data.ReasonforNonUtilization);
                    formik.setFieldValue('Lifeleft', data.Lifeleftindays);
                    formik.setFieldValue('Remarks', data.Remarks);
                    onChangeRequestTypeNew(data.PlantCodeId);
                  }
                }, [workrequestColl])                
              }
          <div>
          ...
          <select id='ddlProjectCodeId' className='form-control' {...getFieldProps(formik, 'ProjectCodeId')} onChange={async (e) => {          
          ...
          <select id='ddlItemCode' className='form-control' {...getFieldProps(formik, 'ItemCodeId')} onChange={async (e) => {          
          ...
          onSelectDate={(date) => formik.setFieldValue('Expiry Date', date?.toISOString())}
